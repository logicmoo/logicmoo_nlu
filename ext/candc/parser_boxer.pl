...
assert_drsdata_0(E):-
   ...
   assert_if_new(used_drsdata(DRS,E)), 
   functor(E,F,A),
   ignore((toplevel_drsdata(DRS),trace,portray_clause(((pred(F,A):-E))))),!.
...
retractall_drs(DRS):-ignore((not(toplevel_drsdata(DRS)),retractall(current_drs(DRS)),retractall(redundant_drsdata(DRS,_)),retractall(used_drsdata(DRS,_)),retractall(shown_drsdata(DRS,_)))).
