...
assert_drs(isaVarPos(Var,Male,Pos)):- assert_drs_holds(isa(Var,Male)), 
  once(pos2ColFn_pred(Pos,POSCol)),assert_drs_holds(isa(Var,POSCol)).
...
retractall_drs(DRS):-retractall(current_drs(DRS)),retractall(redundant_drsdata(DRS,_)),retractall(used_drsdata(DRS,_)),retractall(shown_drsdata(DRS,_)).
