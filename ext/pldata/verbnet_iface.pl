...
atrace:-trace.
useCateID:- fail.
:- abolish(verbnet_pred/1).
:- abolish(verbnet_type/1).
:- abolish(verbnet_type/2).
ppfs:- cls, make  , ppfs('verbnet-3.2/*.xml'), listing(verbnet_pred),listing(verbnet_type).
       ...
       ('format'('%% skipping: ~q ~n',[Elis]),flush_output(user_output))
        ; 
         ...
         setup_call_cleanup(open(Elis,write,Str),
          writeEachTo(no_parent,[],Str,Z),close(Str))))).

splt_g(Atom,G):-atomic_list_concat(G,' ',Atom),G\==[],G\==[''],!.
splt_g('',[]).
...
writeEachTo(ID,Ctx,Str,N=V):- crrect_value_arg(N,V,VV), !, atom_concat('verbnet_',N,Pred), P=..[Pred,ID,VV],!,bothPlaces(ID,Ctx,Str,'~N~q.~n',[P]).


writeEachTo(ID,Ctx,Str,element(Tag,[],STUFF)):- member(Tag,['FRAMES','MEMBERS','EXAMPLES']),!,writeEachTo(ID,Ctx,Str,STUFF).

writeEachTo(ID,_Ctx,Str,element('MEMBER',S,[])):- 
  select(name=Name,S,_),atom_string(Name,WStr),writeEachTo(ID,t(verbnet_word(Name),ID),Str,WStr),
   \+ \+ ((select(grouping=Gs,S,_),splt_g(Gs,GsL),must_maplist(writeEachTo(ID,t(verbnet_map_fn(Name),ID),Str),GsL))),
   \+ \+ ((select(wn=Gs,S,_),splt_g(Gs,GsL),must_maplist(writeEachTo(ID,t(verbnet_map_wn(Name),ID),Str),GsL))),!.


writeEachTo(ParentID,Ctx,Str,element(CLASS,S,STUFF)):- % member(CLASS,['VNCLASS','VNSUBCLASS']), !,   
   member('ID'=ID,S),!,
   flag(frame_num,_,0),
   bothPlaces(ID,Ctx,Str,"~N~n~q.~n",[verbnet_class(ID,CLASS,ParentID,Ctx)]),!,
   must((select(element('MEMBERS',[],WORDS),STUFF,STUFF1),writeEachTo(ID,[ID|Ctx],Str,WORDS),
   must((select(element('THEMROLES',[],SS),STUFF1,STUFF2),
                       simplify_seme('THEMROLES',SS,SSS),
                       writeEachTo(ID,[ID|Ctx],Str,SSS))),
   select(element('FRAMES',[],FRAMES),STUFF2,STUFF3),
      writeEachTo(ID,[ID|Ctx],Str,element('FRAMES',[],FRAMES)),
   select(element('SUBCLASSES',[],SUBCLASSES),STUFF3,STUFF4),writeEachTo(ID,[ID|Ctx],Str,SUBCLASSES),
   STUFF4==[])),!.


%writeEachTo(ID,Ctx,Str,element('FRAMES',[],[element('FRAME',[],STUFF)])):- !, must(writeEachTo(ID,Ctx,Str,STUFF)).
writeEachTo(ID,Ctx,Str,element('FRAMES',[],LIST)):- flag(frame_num,_,0),!, writeEachTo(ID,Ctx,Str,LIST).

writeEachTo(ID,Ctx,Str,element('FRAME',[],STUFF)):- !,
  flag(frame_num,N,N+1),
  (N==0->atomic_list_concat([ID,'_f',N],NewID);atomic_list_concat([ID,'_f',N],NewID)),
  writeEachTo(NewID,Ctx,Str,STUFF).
writeEachTo(ID,Ctx,Str,element('DESCRIPTION',S,[])):- !, maplist(writeEachTo(ID,Ctx,Str),S).
writeEachTo(ID,Ctx,Str,element('EXAMPLE',[],[S])):-!, writeEachTo(ID,Ctx,Str,example=S),!.
writeEachTo(ID,Ctx,Str,element('SYNTAX',[],SS)):- simplify_syns('SYNTAX',SS,SSS), writeEachTo(ID,Ctx,Str,syntax=SSS).
writeEachTo(ID,Ctx,Str,element('SEMANTICS',[],SS)):- simplify_seme('SEMANTICS',SS,SSS), writeEachTo(ID,Ctx,Str,semantics=SSS).


writeEachTo(ID,Ctx,Str,element('THEMROLES',[],SS)):-  simplify_seme('THEMROLES',SS,SSS),
  append_terms(frame_info(themroles),ID,SC),!, 
  append_term(SC,SSS,SSSS), bothPlaces(ID,Ctx,Str,"~N~q.~n",[SSSS]),!.

/*
writeEachTo(ID,Ctx,Str,element('PRED',[bool=!,value=V],SS)):- simplify_syn(Kind,SS,SSS),append_terms(frame_info(logical_form),Ctx,SC),!, 
  append_terms(SC,[not(V),SSS],SSSS), bothPlaces(ID,Ctx,Str,"~N~q.~n",[SSSS]).
writeEachTo(ID,Ctx,Str,element('PRED',[value=V],SS)):- simplify_syns('SYNTAX',SS,SSS),append_terms(frame_info(logical_form),Ctx,SC),!, 
  append_terms(SC,[V,SSS],SSSS), 
  bothPlaces(ID,Ctx,Str,"~N~q.~n",[SSSS]).
*/
...
simplify_sem(element('PRED',[bool=(!),value=Cause],[args(ARGS)]),~(P)) :- P=..[Cause|ARGS],!,
  length(ARGS,A),assert_if_new(verbnet_pred(Cause,A)).
...
simplify_sem(element(_,['Value'=(+),type=V],[]), +V):- assert_if_new(verbnet_type(V)).
simplify_sem(element(_,['Value'=(-),type=V],[]), -V):- assert_if_new(verbnet_type(V)).
simplify_sem(element(W,[type=V],[]), V):- maybe_type(W,V).
simplify_sem(STUFF,STUFFY):- STUFF=..List,maplist(simplify_sem,List,ListO),STUFFY=..ListO.

maybe_type(_,V):- atom_contains(V,?),!.
maybe_type(_,V):- atom_contains(V,"("),!.
maybe_type(W,V):- assert_if_new(verbnet_type(V,W)).
