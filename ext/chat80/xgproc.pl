...
abolish_xg(Prop):- 
  ...
  doall((user:current_xg_pred(M,F,A,Props),
                 member(Prop,Props),                 
                 ignore((memberchk(xg_pred=P,Props),dmsg(abolising(current_xg_pred(M,F,A,Props))),
                         predicate_property(P,number_of_clauses(NC)),flag(xg_assertions,XAC,XAC-NC))),
                 M:abolish(F,A),
                 dynamic(M:F/A),
                 retractall(user:current_xg_pred(M,F,A,_)))).
...
%   U is H-Hf-H0+Hf0,
    U is H-H0,
   dfmt('~N** Grammar from file ~w: ~w words .. time ~w **~n~n',[F,_W,U]).
...
xg_process_now(X,Mode):- catch(xg_process(X,Mode),Error,(dmsg(Error),fail)),!.
