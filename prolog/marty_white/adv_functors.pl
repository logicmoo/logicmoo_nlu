...
type_functor(unk,precept_local(A,departing(B,C,A,D,E))).
type_functor(unk,b(A,[precept_local(B,departing(C,D,B,E,F)),in(A,G)])).
...
type_functor(unk,bugout1(failed_update_model(A,B,C),model)).
...
type_functor(unk,precept(A,B,C,D)).
