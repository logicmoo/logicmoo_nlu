...
bratko_parse(Sentence, LF):-  utterance(_How, LF, Sentence, []).
...
dcg_push(A,S,[A|S]).
%dcg_push(A,S,S):- S=[H|T],NewS=[H|T],setarg(2,S,NewS),setarg(1,S,A),!.

optionalText1(X) --> { length(X, L), L > 0, L < 33 } , X.
optionalText1(_) --> [].

dcg_s2(DCG2, In, Out) --> {append_term(DCG2, In, DCG1), append_term(DCG1, Mid, DCG0)}, DCG0, dcg_s2(DCG2, Mid, Out).
dcg_s2(_, Same, Same) --> [].
...
% =================================================================
declarative(LFOut) --> sentence(_Frame, LFOut), optionalText1([.]).
...
% "How are you?"
% "How will you?"
interogative(LFOut => answer(X))   -->  whpron, verb_phrase(_NewFrame, X, LFOut). % verb Form be _Tense+fin
% "where is joe walking?"
interogative(LFOut => answer(X))   -->  whpron, sentence_inv(X, LFOut).  % was gap(noun_phrase, X)
...
rel_clause(X, Prop1, Prop1 & Prop2) --> {relpron(Who)},[Who], verb_phrase(_NewFrame, X, Prop2).
...
noun_lf(X, Sing,  ISA) :- noun_lf1(Sing), into_isa3(X, Sing, ISA).
