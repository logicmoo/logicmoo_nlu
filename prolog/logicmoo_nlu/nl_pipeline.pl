...
:- abolish(apply_macros:expand_apply,2).
apply_macros:expand_apply(_In,_):- fail.
:- abolish(apply_macros:expand_apply,4).
apply_macros:expand_apply(_In,_,_,_):- fail.
...
try_converter(TID,CNV):- strip_module(CNV,M,CNVLST), CNVLST=..[F|Args],!,
  ignore((on_x_debug(maplist(parser_all:make_io_closure(TID),Args,IOArgs,CLOSURES)), 
                     IOCNVLST=..[F|IOArgs],!, 
                     show_call(call('@'(IOCNVLST,M))),must_maplist(must,CLOSURES))).
...
make_io_closure(TID, +(Name,Else),ValueO,true):-!, get_pipeline_value(TID,Name,Value,Else).
...
%
run_pipeline(StartingNameValues0,WaitingOnNVs0,RAllNameValues):- 
    setup_call_cleanup(
      notrace(
...
text_pipeline(AceText,AllNameValues):- 
  run_pipeline([acetext=AceText],[untildone=_],AllNameValues).
...
run_pipeline_id(_TID,_WaitingOnNVs, overflow(N), N):- N> 10, !. 
...
run_pipeline_id( TID, WaitingOnNVs, ExitWhy, N):- 
    flag(TID,_,0),
    forall(choose_converter(TID,WaitingOnNVs,M,CNV), try_converter(TID,M:CNV)),
    N2 is N +1,
...
choose_converter(TID,WaitingOnNVs,M,CNV):- installed_converter(M,CNV),
   \+ \+ ((sub_term(Sub,CNV),(tl:pipeline_value(TID,N,_);member(N=_,WaitingOnNVs)),N==Sub)).
...
:- install_converter(parser_chat80:sent_to_parsed(+ tokens /*acetext_no_punct*/ ,-syntaxTree80)).
...
% ================================================================================================
% English2CycL:  
:-  if(load_parser_interface(parser_e2c)). % TODO confirm CHAT80 runs without E2C
% ================================================================================================
...
% ================================================================================================
%:-  load_parser_interface(parser_talk).
% ================================================================================================
...
:-  load_parser_interface(parser_CURT).
% ================================================================================================
...
baseKB:sanity_test:- run_pipeline(acetext='A person who loves all animals is loved by someone.',[kif(p)=_],O),wdmsg(O).
baseKB:sanity_test:- run_pipeline(acetext='All persons are happy.',[kif(p)=_],O),wdmsg(O).
baseKB:regression_test:- run_pipeline(acetext='What is the ocean that borders african countries and, that borders asian countries?',[qplan=_],O),wdmsg(O).
baseKB:regression_test:- run_pipeline(acetext='what countries are there in europe ?',[qplan=_],O),wdmsg(O).
baseKB:regression_test:- must_test_80(Tokens,_,_),run_pipeline([tokens=Tokens],[qplan=_],O),wdmsg(O).
baseKB:regression_test_TODO:- run_pipeline(acetext='A person who loves all animals is loved by someone.',[kif(p)=_],O),wdmsg(O).
...
:- listing(sanity_test/0).
:- listing(regression_test/0).

:- fixup_exports.

:- forall(must_test_80(Tokens,_,_),(run_pipeline([tokens=Tokens],[results80=_],O),maplist(wdmsg,O))).
