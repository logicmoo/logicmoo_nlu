...
baseKB:sanity_test :-
    ...
    show_kvs(O).
baseKB:sanity_test :-
    run_pipeline(input='All persons are happy.', [kif(p)=_], O),
    wdmsg(O).
...
:- dynamic baseKB:regression_test/0.
:- multifile baseKB:regression_test/0.
:- module_transparent baseKB:regression_test/0.

baseKB:regression_test :-
    test_chat80_regressions.
baseKB:regression_test :-
    run_pipeline('What are the oceans that border african countries and that border asian countries ?').
baseKB:regression_test :-
    run_pipeline('What is the ocean that border african countries and that border asian countries?',
                 [qplan=_],
                 O),
    wdmsg(O).
baseKB:regression_test :-
    run_pipeline(input='what countries are there in europe ?',
                 [qplan=_],
                 O),
    show_kvs(O).
baseKB:regression_test :-
    must_test_80(Tokens, _, _),
    run_pipeline([tokens=Tokens], [qplan=_], O),
    show_kvs(O).
baseKB:regression_test :-
    animals_test.
baseKB:regression_test :-
...
chat80(A) :-
    var(A),
    must_test_80(B, _, _),
    once(( tokens_to_acetext(B, C),
           any_to_string(C, A)
         )).
chat80(A) :-
    var(A),
    clause(chat80([B|C]), true),
    once(( tokens_to_acetext([B|C], D),
           any_to_string(D, A)
         )).
chat80(A) :-
    var(A),
    clause(chat80(A, _, _), true).
chat80(A) :-
    var(A),
    clause(chat80(A, _), true).
...
chat80(A, B) :-
    var(A),
    must_test_80(D, C, _),
    once(( member(answers(B), C),
           tokens_to_acetext(D, E),
           any_to_string(E, A)
         )).
